...
2) Step functions: When critical variables exceed their acceptable limits, the secondary loop triggers discrete, step-like changes to the system's parameters or structure - essentially reorganizing how the system operates.    

3) Self-organization: The system randomly or systematically tries different configurations until it finds one that keeps essential variables stable. Once a stable configuration is found, it persists until the environment changes enough to destabilize it again.    
...
The key difference: McCullough tended toward a more neurological and computational perspective where feedback enables complex behaviors and calculations, while Ashby focused on feedback as a regulatory principle for stability and adaptation. McCullough explored what feedback makes possible; Ashby formalized what feedback must do to maintain viability.
Both were central figures in early cybernetics, but Ashby's approach was more abstractly mathematical while McCullough's remained closer to biological implementation.    
